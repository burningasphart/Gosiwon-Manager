...
# --- [2] 핵심 기술: 행 튀어오름 방지 및 실시간 저장 콜백 ---
def handle_edit():
    """편집기 내용이 바뀔 때만 안전하게 실행됨 (행 고정 핵심)"""
    # 에디터가 세션에 존재할 때만 실행 (KeyError 방지)
...
with tabs[-2]: # 데이터 편집
    ...
    st.info("💡 수정 시 행 위치가 완벽히 고정됩니다. 다른 탭을 이동해도 데이터가 유지됩니다.")
    cat_list = st.session_state.cat_df["항목명"].tolist()
    
    # 핵심: on_change 콜백으로 에러 없이 실시간 동기화
    st.data_editor(
