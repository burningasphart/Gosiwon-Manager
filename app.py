...
with tabs[1]: # 편집 탭 (행 고정 기술 적용)
    st.subheader("📝 장부 통합 편집")
    st.info("💡 수정 후 하단의 [저장] 버튼을 누르면 모든 월별 탭에 반영됩니다.")
...
with tabs[0]: # 리포트 탭
    if not df.empty:
        ...
        total_expense = df[df['구분']=='비용']['금액'].sum()
        st.metric("누적 순이익", f"{total_income - total_expense:,}원")
        st.plotly_chart(px.bar(df, x='월', y='금액', color='구분', barmode='group'), use_container_width=True)
    else:
        st.info("사이드바에서 파일 2개를 업로드하고 '합치기'를 눌러주세요.")
