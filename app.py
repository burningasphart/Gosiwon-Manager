...
# --- [2] 보조 함수: 2025년 9~12월 장부 기반 초정밀 키워드 보강 ---
def smart_categorize(content, is_income):
    ...
    # 2. 공과금 (장부 내 SKB, 세무회계, 보험 등 반영)
    if any(k in text for k in ['전기', '수도', '예스코', '가스', '한전', '공과금', 'SKB', 'SK인터넷', '인터넷', '세무', '부가세', '보험', 'ETAX', '대출', '수도요금', '소득세', '지방세']): 
        return "공과금"
    
    # 3. 식품 (햇반, 오뚜기, 사리면, 진라면, 하늘보리 등 실제 구매 품목)
    if any(k in text for k in ['쌀', '라면', '사리면', '진라면', '신라면', '햇반', '오뚜기', '아몬드', '곰곰', '바나나', '포카리', '하늘보리', '삼다수', '커피', '종이컵', '설탕']): 
        return "식품"
    
    # 4. 비품 (다이소, 매트리스 커버, 세탁세제, 형광등 등)
    if any(k in text for k in ['다이소', '아성다', '비품', '세제', '휴지', '점보롤', '타월', '세탁', '봉투', '종량제', '매트리스', '형광등', '건전지', '샤워기', '뚫어뻥', '슬리퍼', '니트릴']): 
        return "비품"
...
prev_master = st.sidebar.file_uploader("과거 마스터 파일(.csv)", type=['csv'])
...
if st.session_state.needs_download:
    ...
    st.download_button("✅ 지금 파일로 저장", st.session_state.master_df.to_csv(index=False).encode('utf-8-sig'), f"율곡고시원장부_{time.strftime('%Y%m%d')}.csv", "text/csv", on_click=lambda: st.session_state.update({"needs_download": False}))
