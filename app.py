...
def process_bank(file, biz_name):
    try:
        ...
        for _, r in df.iterrows():
            ...
            rows.append({"연도":y, "월":m, "날짜":d, "사업장":biz_name, "내용":cont, "용도":final_usage, "구분":TYPE_MAP.get(final_usage, "비용"), "금액":vi if vi>0 else vo, "비고":""})
        return rows
    except: return []
...
with t2: # 편집 탭
    ...
    if not target_df.empty:
        ...
        st.markdown(f'<div style="background-color:#f0f2f6;padding:15px;border-radius:10px;margin-bottom:15px;"><span style="color:red;font-weight:bold;font-size:1.1rem;">🔴 수익 합계: {int(in_s):,}원</span> | <span style="color:blue;font-weight:bold;font-size:1.1rem;">🔵 비용 합계: {int(ex_s):,}원</span> | <b style="font-size:1.1rem;">💰 현재 합계: {int(in_s-ex_s):,}원</b></div>', unsafe_allow_html=True)

        # --- [색상 핵심 로직: 배경색까지 입혀서 확실하게 구분] ---
        def apply_row_style(row):
            # '구분' 열의 값에 따라 스타일 결정
            if row['구분'] == '수익':
                style = 'background-color: #ffeaea; color: red; font-weight: bold'
            elif row['구분'] == '비용':
                style = 'background-color: #eaf2ff; color: blue; font-weight: bold'
            else:
                style = ''
            
            # '구분' 열에만 스타일 적용
            return [style if name == '구분' else '' for name in row.index]

        # 편집창에 스타일 적용된 데이터 전달
        edited = st.data_editor(filtered_df.style.apply(apply_row_style, axis=1), use_container_width=True, num_rows="dynamic",
...
with t3: # 설정 탭 (규칙 관리 유지)
    st.subheader("⚙️ 자동 분류 규칙 관리")
    with st.expander("➕ 새 규칙 추가", expanded=True):
        sc1, sc2, sc3 = st.columns([2, 2, 1])
        with sc1: kw = st.text_input("키워드")
        with sc2: ct = st.selectbox("용도", options=CAT_LIST)
        with sc3:
            if st.button("규칙 추가"):
                if kw:
